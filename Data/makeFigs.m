function makeFigs(data, name, subjectCode, date, color, divLim, pointSlope, logPlot)
    divided = figure();
    distribution = figure();
    ...
    title(sprintf("Letter Height/Eccentricity vs. Eccentricity (%s %s) (%s)", ...
        name, subjectCode, date), 'FontSize', 14);
...
%     binWidth = (max(data(:,2)')/25);
%     upperEdge = (max(data(:,2)')*1.2)/binWidth;
%     edges = [];
%     for i = 1:upperEdge
...
%     box on;
    ...
    title(sprintf(titleText, name, subjectCode, date), 'FontSize', 12);
    ...
    if(strcmp(name,'Anstis') == 0)
        ...
        if(exist(csvName, 'file') ~= 2)
            ...
            fprintf(fileID, '%s, %s, %s, %s, %s, %s, %s\n', 'Subject', 'Date', ...
                'Protocol', 'Truncated Mean', 'Truncated SD', 'Truncated N', ...
                'Standard Error');
            fclose(fileID);
        end
        ...
        fileID = fopen(csvName, 'a');
        fprintf(fileID, '%s, %s, %s, %5.4f, %5.4f, %d, %7.6f\n', ...
            subjectCode, date, name, avg, sd, N, sError);
        fclose(fileID);
