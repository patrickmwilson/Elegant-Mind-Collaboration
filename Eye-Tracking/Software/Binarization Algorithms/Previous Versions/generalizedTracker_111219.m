function [allObjectStats, objectsLost] = genEyeTracking();
    clear all;
    close all;
    ...
    folderName = 'Trevor_Calibration_110519';
    mkdir(folderName);
    cd(folderName);

    numThresh = 50;
    resizeBox = [-20, -20, 2*20, 2*20]; 
    ...
    if strcmp(fileType, '\*.tiff') || strcmp(fileType, '\*.tif')
        ...
        for ii = 1:numObjects;
            ...
            prevLocs(ii, :) = objectStats(ii).Centroid;
        end
...
%                     end                
        ...
        save(strcat(folderName, '.mat'), 'allObjectStats', 'objectsLost', 'storedCentroids');    
